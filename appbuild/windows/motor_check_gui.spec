...
"""
PyInstaller spec — Windows EXE 빌드용.

실행 위치: appbuild/windows/ (프로젝트 루트에서 상대경로 ../../로 접근)
사용법: (appbuild/windows/ 디렉토리에서)
    pyinstaller motor_check_gui.spec
"""
import os

# 프로젝트 루트 경로 (appbuild/windows 기준 두 단계 위)
...
exe = EXE(
    ...
    icon=os.path.join(PROJECT_ROOT, 'resource', 'icon.ico'),
)
