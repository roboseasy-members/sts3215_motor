...
```
┌──────────────────────────────────────────────────────┐
│ Auth Gate — main.py 외부 루프                         │
│   ui/welcome_dialog.py       (구글 로그인 화면)        │
│   auth/google_oauth.py       (OAuth flow + 토큰 저장)  │
└───────────────┬──────────────────────────────────────┘
                │ (로그인 성공 시 진입)
┌───────────────▼──────────────────────────────────────┐
│ UI Layer — PyQt6                                     │
...
4. **외부 루프 (로그인 재진입용)**:
   - 저장된 토큰으로 `load_saved_credentials()` → 성공 시 `fetch_user_info()` 로 프로필 조회 (자동 로그인)
   - 실패/토큰 없음 → `WelcomeDialog` 노출, 사용자가 구글 로그인 완료해야 진행
   - 로그인 취소 시 `sys.exit(0)` — 오프라인/스킵 모드 없음
5. **내부 루프 (모드 선택 + 창 실행)**:
   - `ModeSelectDialog.exec()` 로 모드 선택
   - `MODE_LOGOUT` 이면 `sign_out()` 후 내부 루프 탈출 → 외부 루프가 `WelcomeDialog` 재표시
   - 그 외 모드는 해당 윈도우 `show()`, `back_to_menu` 시그널 대기
   - 뒤로가기가 발생하면 내부 루프 재진입, 아니면 종료

이 2 중 루프 구조 덕분에 (a) 각 창 → 모드 선택, (b) 로그아웃 → 웰컴 화면 복귀가 자연스럽게 동작.
